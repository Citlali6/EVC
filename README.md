...
当前训练代码结束时会打印三个权重路径。常规 50 epoch 基线训练暂时使用 `best_iou_seed37.pt`；1 epoch 烟雾测试使用 `best_loss_seed37.pt`。完成后文的比赛得分验证改造后，比赛模型应优先使用新增的 `best_score_seed37.pt`。评估或提交前，将对应绝对 WSL 路径填入所用 YAML 的 `TEST.model_path`，例如：
...
| `configs/evisseg_evuav_challenge2.yaml` | `TEST.prediction_threshold` | 保持官方测试脚本使用的固定阈值 `0.9`，`test2.py` 与 `submit_challenge2.py` 必须使用同一个值。 | 不要将它作为调参或优化项。 |
...
官方测试脚本使用的预测阈值 `0.9` 不是本项目的优化变量。所有基线、消融实验和提交结果都固定使用 `0.9`，同时保持 `pd_detT: 50`、`correct_thresh: 0.0001` 和官方得分公式不变。改进必须来自模型、训练策略或合法后处理，而不是修改评测条件。
...
| --- | --- | --- | --- | --- | --- |
| P0 | 后处理 | 时空虚警簇过滤 | **最值得先试。** `Fa` 按虚警连通区域计数并指数惩罚，时空孤立预测与失分直接相关。 | 在固定阈值 `0.9` 后，只删除事件数过少的时空孤立簇；验证有效后，再单独加入最短持续时间条件。 | 第一版不补全断轨迹，因为新增前景可能提高 `Fa`；不同时调整多个过滤参数。 |
| P1 | 损失函数 | 虚警感知损失与背景难例挖掘 | **值得尝试。** 当前 STC Loss 对负样本使用 `1 - stc_weight`，局部密集的背景簇可能得到较低负样本惩罚，而这类错误容易形成虚警。 | 保留原 STC Loss，只增加一个高置信背景难例 BCE 项，优先惩罚训练集中的错误背景预测。 | 不直接堆叠 Focal、Tversky、类别平衡和难例挖掘；单纯大幅提高正样本权重可能增加 `Fa`。 |
| P1 | 训练采样策略 | 目标保持与时间分层采样 | **值得作为 100000 事件训练的独立方向。** 当前均匀随机采样可能丢失稀少目标事件并破坏时间连续性。 | 仅在训练集使用标签保护目标事件，背景事件按时间片分层采样，并保持总事件预算不变。 | 验证和测试阶段禁止标签采样；若收益不能迁移到更大事件预算，则只把它作为低显存训练手段。 |
| P2 | 损失函数 | 多尺度或自适应 STC 邻域 | **方向合理，但晚于损失重加权。** 固定 `k=3, t=5` 对不同速度和事件密度的目标适应性有限。 | 在少量离散邻域之间进行轻量门控，逐项比较空间邻域和时间邻域，不直接预测任意大小。 | 不与新的背景难例损失同时首次引入，否则无法判断收益来源。 |
| P2 | 特征提取 | 密度条件化 GDSCA | **可以尝试，但预期成本高于前述方向。** 当前四个分支的空洞率固定为 `[1, 2, 3, 4]`。 | 保留现有卷积分支，只根据局部事件密度学习轻量分支权重。 | 暂不实现自由偏移的可变形稀疏卷积，其自定义算子、显存和兼容风险较高。 |
| P3 | 多表示融合 | 点云与事件帧轻量融合 | **有研究价值，但目前证据不足。** 它可能改善强噪声和极稀疏场景，也会显著增加参数、显存和实验变量。 | 只有错误分析确认单一点云表示在特定场景持续失败后，再增加轻量事件帧分支和门控融合。 | 不作为第一轮改进，也不与 GDSCA 改造同时进行。 |

### 实验前提

`train.py` 目前从第 40 个 epoch 起只按验证集 `IoU` 保存 `best_iou`，但比赛按完整 `Score` 排名。因此开始算法消融前，需要先让训练验证阶段计算 `Pd/Fa/IoU/Acc/Score` 并保存 `best_score`。这只是正确选择权重的实验基础，不算新的算法贡献，也不能和某个改进方向混为一次实验。

后处理需要由 `test2.py` 和 `submit_challenge2.py` 调用同一个实现，固定阈值仍为官方的 `0.9`，从而保证本地验证结果与最终提交文本一致。

### 推荐实施顺序

1. **评测准备**：复现固定阈值 `0.9` 的基线，并增加 `best_score` 权重保存逻辑，不改变算法。
2. **P0 后处理实验**：只增加“时空簇最小事件数”过滤，与无后处理基线比较；通过后再单独测试持续时间过滤。
3. **P1 损失函数实验**：关闭后处理，只增加一个背景难例损失；它独立有效后，再与 P0 最优方案叠加。
4. **P1 训练采样实验**：恢复原损失和无后处理，只替换训练采样；独立有效后再叠加。
5. **P2 损失函数与特征提取实验**：依次验证自适应 STC 和密度条件化 GDSCA，每次只选择一个。
6. **P3 多表示融合实验**：只有前述方向达到瓶颈且错误分析提供明确依据时，再尝试多表示融合或可变形稀疏卷积。
...
6. 官方预测阈值固定为 `0.9`，不是实验变量。调后处理时固定模型、损失和采样；调模型时关闭后处理或固定为已经确认的版本。
